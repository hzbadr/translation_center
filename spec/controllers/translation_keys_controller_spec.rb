...
module TranslationCenter
    ...
    routes { TranslationCenter::Engine.routes }
	  ...
	  before(:each) do
 	  ...
 	  describe "POST update_translation" do
 	    it "should assigns @translation & render nothing" do
 	  	  post :update_translation, translation_key_id: translation_key.id, format: :json
 	  	  expect(assigns(:translation_key)).to eq translation_key
 	  	  expect(response.body).to be_blank
 	    end

      it "should assigns @translation & render json" do
 	  	  post :update_translation, translation_key_id: translation_key.id, value: "value value", format: :json 
 	  	  expect(assigns(:translation_key)).to eq translation_key
 	    end
 	  end
	  ...
	  describe "GET translations" do
 	    it "should assigns @translations sorted by votes & render translations.js" do
		  FactoryGirl.create(:translation, translation_key: translation_key)
 	  	get :translations, translation_key_id: translation_key.id, sort_by: "votes", format: :js
 	  ...
 	  it "should assigns @translations sorted by created_at" do
 	  	FactoryGirl.create(:translation, translation_key: translation_key)
 	  	get :translations, translation_key_id: translation_key.id, format: :js
 	  	expect(assigns(:translations).first.class.name).to eq "TranslationCenter::Translation"
 	  end
 	end
 	...
 	describe "GET show" do
  	it "should assign @translation_key & render show" do
  	  get :show, id: translation_key.id
  	  expect(assigns(:translation_key)).to eq translation_key
  ...
  describe "PUT update" do
  	it "should assign @translation_key" do
  	  put :update, id: translation_key.id, format: :json
  	  expect(assigns(:translation_key)).to eq translation_key
  	  expect(TranslationKey.find(translation_key.id).name).to eq translation_key.name
  	end

  	it "should assign @translation_key & update value" do
  	  put :update, id: translation_key.id, format: :json, value: "new value"
  	  expect(TranslationKey.find(translation_key.id).name).to eq "new value"
  	end
  end

  describe "POST destroy" do
  	it "should assign translation & render destroy.js" do
  	  delete :destroy, id: translation_key.id, format: :js
